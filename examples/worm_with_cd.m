...
do_cd = true;
ym = 500 ; % youngs modulus stiffness
max_steps = 100; 
...
name = 'worm';
%% Load Rig stuff
[V0, F, W, P0] = read_rig_from_json(strcat("./data/", name, "/rig.json"), d=2);
P = read_anim_from_json(strcat("./data/", name, "/anim.json"), d=2)
...
if(save_video)
    v = VideoWriter(strcat(name, '_cd.mp4'),'MPEG-4');
    v.Quality = 100;
    v.FrameRate = 24;
    open(v)
end;
...
writeOBJSequence(strcat("./", name,"_dolphin_recording/"), Vs', F, d=2)
